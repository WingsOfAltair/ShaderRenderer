...
vec3 sky(vec3 rd)
{
    ...
    vec3 horizon = vec3(0.25, 0.3, 0.35);

    return mix(top, mix(mid, horizon, t), t);
}
...
void main()
{
    ...
    if (i < 64)
    {
        ...
        // BIOME COLOR
        vec3 base = biomeColor(h);

        float diff = max(dot(n, normalize(vec3(0.3,1.0,0.2))), 0.0);

        col = base * (0.3 + diff);

        // WATER reflection boost (low terrain)
        float waterMask = smoothstep(-0.5, 0.5, -h);
        col += vec3(0.2, 0.4, 0.8) * waterMask * 0.5;
